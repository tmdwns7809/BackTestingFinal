...
namespace BackTestingFinal
{
    public sealed class FundingRateBackTesting : BaseFunctions
    {
        ...
        public Button MainChartButton = new Button();
        ...
        void SetAdditionalMainView()
        {
            ...
            SetButton(MainChartButton, "Main", (sender, e) => { charButton(mainChart, MainChartButton); });
            MainChartButton.Size = new Size(40, 20);
            MainChartButton.Location = new Point(mainChart.Location.X + mainChart.Width, mainChart.Location.Y + 10);

            SetButton(Buttons[2], "T", (sender, e) => { charButton(Charts[2], Buttons[2]); });
            Buttons[2].Size = MainChartButton.Size;
            Buttons[2].Location = new Point(MainChartButton.Location.X, MainChartButton.Location.Y + MainChartButton.Height + 35);
        ...
        {
            var list = showingItemData.listForFundingRate.list;
            ...
            form.Text = showingItemData.Code + "  C:" + list[i].Price[3];
        }

        ///////////////////

        public void SetChartNowOrLoad(int position = int.MinValue, bool loadNew = false)
        {
            ...
            var from = mainChart.Tag != null ? GetStandardDate(chartValues: ChartTimeSet.EightHour) : default;
        ...
        void ShowChart(BackItemData itemData, (DateTime time, int position, bool on) cursor, bool loaded = false, ChartValues chartValues = default)
        {
            if (chartValues == default)
                chartValues = ChartTimeSet.EightHour;
        ...
        (DateTime foundTime, ChartValues chartValues) LoadAndCheckSticks
            (BackItemData itemData, bool newLoad, bool toPast, int minSize = default
            , DateTime from = default, ChartValues chartValues = default, bool oneChart = true)
        {
                    ...
                    for (int j = (int)Position.Long; j <= (int)Position.Short; j++)
                    {
                        ...
                        else if (positionData.Enter)
                        {
                                ...
                                itemData.firstLastMin.lastMin == from2)
                            {
                                ...
                                {
                                    if (cl.Key.index > ChartTimeSet.OneMinute.index && cl.Key.index < Strategy.minCV.index)
                                        continue;
        ...
        void CalculateMetric(DateTime start, DateTime end, Position isAllLongShort)
        {
            ...
            var d = ChartTimeSet.OneDay.seconds / Strategy.DetailCV.seconds;
        ...
        bool CheckAndCompliteFromTheMinuite(BackItemData itemData, DateTime from2, DateTime tst)
        {
            ...
            try
            {
                ...
                for (int j = ChartTimeSet.OneMinute.index; j <= Strategy.maxCV.index; j++)
                {
                    if (j != ChartTimeSet.OneMinute.index && j < Strategy.minCV.index)
                        continue;
                    ...
                    if (from2.TimeOfDay == TimeSpan.Zero)
                    {
                        if (v.CLD.list.Count == 0)
                        {
                            ...
                            {
                                v.CLD.list = LoadSticks(itemData, v.CV, from2.AddSeconds(-v.CV.seconds * (Strategy.IndNeedDays + Strategy.CompareNeedDays - 1)),
